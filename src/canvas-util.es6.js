class CanvasUtil {
  ...
  static getPixelRedFromImageData(imageData, contextWidth, x, y) {
    var rIndex = (y * contextWidth + x) * 4;
    return imageData[rIndex];
  }
  ...
  static loadImageToContext(imagePath, callback) {
    var image = new Image();
    image.onload = () => {
      let canvasCtx = CanvasUtil.imageToContext(image);
      callback(canvasCtx, image);
    };
    image.src = imagePath;
  }

  static imageToContext(img) {
  ...
  static copyPixels(
    ...
    destination.putImageData(
      source.getImageData(sourceX, sourceY, sourceW, sourceH),
      destX,
      destY,
      destX,
      destY,
