...
import LinearFloat from "./linear-float.es6.js";
...
class GestureTracker {
  ...
  constructor(bufferSize, switchFrames, activeThresh=0.5) {
    ...
    this.thresholdSwitches = this.thresholds.map(thresh => { return new LinearFloat(0, switchFrames) });
  ...
  toHtmlString() {
    ...
    return `
      ...
      <div><code>this.inputBuffer.direction</code> = ${this.inputBuffer.average() > 0 ? 'up' : 'down'}</div>
      ...
      <div><code>this.thresholdSwitches()</code> = ${this.thresholdSwitches.map((switchObj, i) => switchObj.value() > 0)}</div>
