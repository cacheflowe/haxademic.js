...
class ThreeSceneAr extends ThreeScene {
  ...
  buildArCameraSource() {
    ...
    let sourceOptions =
      this.cameraSource == ThreeSceneAr.WEBCAM
        ? { sourceType: "webcam" }
  ...
  buildMarkerConfig() {
    ...
    this.markerControls1 = new THREEx.ArMarkerControls(
  ...
  buildLights() {
    // ambient light
    const ambientColor = this.lighting.ambientColor
      ? this.lighting.ambientColor
      : 0xcccccc;
    const ambientIntensity = this.lighting.ambientIntensity
    ...
    // hemisphere light
    const lightHemiSkyColor = this.lighting.lightHemiSkyColor
      ? this.lighting.lightHemiSkyColor
      : 0x999999;
    const lightHemiGroundColor = this.lighting.lightHemiGroundColor
      ? this.lighting.lightHemiGroundColor
      : 0xffffff;
    const lightHemiIntensity = this.lighting.lightHemiIntensity
      ? this.lighting.lightHemiIntensity
      : 0.65;
    this.lightHemisphere = new THREE.HemisphereLight(
      lightHemiSkyColor,
      lightHemiGroundColor,
      lightHemiIntensity
    );
    this.scene.add(this.lightHemisphere);

    // directional light
    const lightDirectionalColor = this.lighting.lightDirectionalColor
      ? this.lighting.lightDirectionalColor
      : 0xffffff;
    const lightDirectionalIntensity = this.lighting.lightDirectionalIntensity
      ? this.lighting.lightDirectionalIntensity
      : 0.5;
    const lightDirectionalX = this.lighting.lightDirectionalX
      ? this.lighting.lightDirectionalX
      : 2;
    const lightDirectionalY = this.lighting.lightDirectionalY
      ? this.lighting.lightDirectionalY
      : 3;
    const lightDirectionalZ = this.lighting.lightDirectionalZ
      ? this.lighting.lightDirectionalZ
      : 1;
    this.lightDirectional = new THREE.DirectionalLight(
      lightDirectionalColor,
      lightDirectionalIntensity
    );
    this.lightDirectional.position.set(
      lightDirectionalX,
      lightDirectionalY,
      lightDirectionalZ
    );
  ...
  buildShadowPlane() {
    // add shadow plane
    const shadowPlaneSize = this.lighting.shadowPlaneSize
      ? this.lighting.shadowPlaneSize
      : 15; // 1 is the size of the AR marker
    const shadowOpacity = this.lighting.shadowOpacity
      ? this.lighting.shadowOpacity
      : 0.5;
    ...
    const shadowLightX = this.lighting.shadowLightX
      ? this.lighting.shadowLightX
      : 1;
    const shadowLightY = this.lighting.shadowLightY
      ? this.lighting.shadowLightY
      : 6;
    const shadowLightZ = this.lighting.shadowLightZ
      ? this.lighting.shadowLightZ
      : 0;
    const shadowSpotlightAngle = this.lighting.shadowSpotlightAngle
      ? this.lighting.shadowSpotlightAngle
      : 0.5;
    const shadowMapSize = this.lighting.shadowMapSize
      ? this.lighting.shadowMapSize
      : 1024;
