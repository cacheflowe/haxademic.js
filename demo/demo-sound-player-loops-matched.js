...
class SoundPlayerDemoLoopsMatched extends DemoBase {
  constructor(parentEl) {
    super(
      parentEl,
      [],
      "SoundPlayer | Loops Matched",
      "sound-player-demo",
      "Synced audio loops!"
    );
  }
  ...
  init() {
    ...
    this.loopIds = [
      "loops/bass-metallic-2.mp3",
      "loops/kicks.mp3",
      "loops/snares.mp3",
      "loops/pet-shop-boys-being-boring-beat.mp3",
    ];
    this.samples = {};
    this.loops = {};
    this.loopVolumes = {};
  ...
  loadedSample(id) {
    ...
    this.loadedCount++;
    if (this.loadedCount == this.sampleIds.length + this.loopIds.length)
      this.allLoaded();
  }
  ...
  loadLoops() {
    ...
    this.el.appendChild(
      DOMUtil.stringToElement(
        "<p>Loops are synced, and have different original lengths. If one is pitched to match the length, it will fall out of sync after a while and would need to be re-triggered</p>"
      )
    );
    this.loopIds.forEach((id) => {
      this.loops[id] = new SoundPlayer(
        "../data/audio/" + id,
        () => {
          this.loadedSample();
        },
    ...
    this.el.appendChild(
      DOMUtil.stringToElement("<p>Toggle the 4 loop channels with a fade.</p>")
    );
    this.loopIds.forEach((id, i) => {
    ...
    // show tick in text
    this.el.appendChild(
      DOMUtil.stringToElement('<p>Tick: <span id="tick">-1</span></p>')
    );
...
if (window.autoInitDemo)
  window.demo = new SoundPlayerDemoLoopsMatched(document.body);
