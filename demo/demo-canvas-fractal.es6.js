...
class CanvasFractalDemo extends DemoBase {
  ...
  constructor(parentEl) {
    super(parentEl, [], 'Canvas | Fractal', 'canvas-fractal-container', 'Press "r" to generate a random hash.<br>Press "s" to save.<br>Add "&hash=FF00AA44....." to the URL to create a link into a deterministic, reproducable shape. Check the console for the current hash.');
  }

  init() {
    let queryHash = Fractal.getQueryParam('hash');
    let hash = (queryHash) ? queryHash : 'c5b841ef41ea44c6682174f9626072c0d47c095665f6e59ad3264bc91f5d2e66';
...
class Fractal {
  ...
  addKeyCommands() {
    window.addEventListener('keydown', (e) => {
      if (e.key == 'r') this.setRandomHash();
      if (e.key == 's') this.saveImage();
    });
  }
  ...
  saveImage() {
    ...
    this.renderedImg.setAttribute('style', "width: 250px; box-shadow: 0 0 5px #000000; position: fixed; bottom: 2rem; left: 50%; margin-left: -125px;");
  ...
  generateShapeProps() {
    ...
    this.fillA = this.remap(this.hexToNum(this.getHashValAt(13)), 0, 255, 0.4, 1);
    this.fillColor = `rgba(${this.fillR},${this.fillG},${this.fillB},${this.fillA})`;
    this.strokeWidth = this.remap(this.hexToNum(this.getHashValAt(5)), 0, 255, 0.5, 10);
    ...
    this.strokeA = this.remap(this.hexToNum(this.getHashValAt(18)), 0, 255, 0.4, 1);
    ...
    this.shadow = this.remap(this.hexToNum(this.getHashValAt(24)), 0, 255, 0, 20);
  }
  ...
  drawPolygonParent(level, x, y, startCircleInc, radius ) {
    ...
    for( var i=0; i < this.polygonVertices; i++ ) {
      ...
      vertices.push(this.drawPolygonChildAtVertex(level, x, y, curRads + startCircleInc, radius ));
    }
