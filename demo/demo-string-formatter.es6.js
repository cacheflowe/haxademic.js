class StringFormatterDemo extends DemoBase {
  ...
  printTests() {
    ...
    this.el.innerHTML = `
      ...
      <div><code>StringFormatter.formatCreditCard('1111-2222-3333-4444')</code> ${StringFormatter.formatCreditCard('1111-2222-3333-4444')}</div>
      <div><code>StringFormatter.formatNumber('$303.33')</code> ${StringFormatter.formatNumber('$303.33')}</div>
      <div><code>StringFormatter.formatDollarsCents('303.333333')</code> ${StringFormatter.formatDollarsCents('303.333333')}</div>
      <div><code>StringFormatter.addCommasToNumber('3000000')</code> ${StringFormatter.addCommasToNumber('3000000')}</div>
      <div><code>StringFormatter.timeFromSeconds(30000, true)</code> ${StringFormatter.timeFromSeconds(30000, true)}</div>
      <div><code>StringFormatter.numberToFormattedString(30000.2102)</code> ${StringFormatter.numberToFormattedString(30000.2102)}</div>
      <div><code>StringFormatter.numberToFormattedCurrency(30303.333333)</code> ${StringFormatter.numberToFormattedCurrency(30303.333333)}</div>
      <div><code>StringFormatter.numberToCompactString(31333, true)</code> ${StringFormatter.numberToCompactString(31333)}</div>
      <div><code>StringFormatter.numberToPercentString(0.1234, 2)</code> ${StringFormatter.numberToPercentString(0.1234, 2)}</div>
    `;
  }

  runUnitTests() {
    let z = zora.test('StringFormatter.formatPhone', test => {
        test.ok(StringFormatter.formatPhone('3035558888'));
        test.equal(StringFormatter.formatPhone('3035558888'), '(303) 555-8888');
      });
      // Promise.all([z]).then((result) => {
      //   console.log('z', z.task, result[0]);
      //   debugger
      // });
    ...
    zora.test('StringFormatter.formatCreditCard', test => {
        ...
        test.equal(StringFormatter.formatCreditCard('1111-2222-3333-4444'), '1111 2222 3333 4444');
      });
