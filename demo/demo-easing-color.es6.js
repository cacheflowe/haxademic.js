...
import EasingColor from '../src/easing-color.es6.js';
import FrameLoop from '../src/frame-loop.es6.js';

class EasingColorDemo extends DemoBase {

  constructor(parentEl) {
    super(parentEl, [], 'EasingColor', 'easing-color-container', 'A lerping color object with lots of hex/rgba/int getters & setters to use your color in the DOM and in WebGL');
  }
  ...
  frameLoop(frameCount) {
    ...
    this.colors.forEach((el, i) => {
      el.update(true);
      html += `
        <div style="background-color: ${el.getRgbaString()}"><code>color ${i+1}</code> : 
          (${Math.round(el.getR())}, ${Math.round(el.getG())}, ${Math.round(el.getB())}) : 
          (${el.getRNorm().toFixed(2)}, ${el.getGNorm().toFixed(2)}, ${el.getBNorm().toFixed(2)}) : 
          ${el.getRgbHex()} : 
          ${el.getRgbaString()}
        </div>
      `;
    ...
    if(frameCount % 200 == 0) {
      this.colors.forEach((el, i) => {
        el.setTargetRgba(this.randomComponent(), this.randomComponent(), this.randomComponent(), 126 + Math.random() * 127);
        if(i % 2 == 0) {
