...
class AudioCueDemo extends DemoBase {
  constructor(parentEl) {
    super(
      ...
      "VTTCue | Audio",
      "audio-cue-container",
      'A simple demo using VTTCues, built programmatically for audio timecode event triggers. Here, each VTTCue\'s "caption" text is stringified JSON.'
    );
  }
  ...
  init() {
    ...
    track.addCue(
      new VTTCue(
        0,
        1,
        JSON.stringify({ cmd: "highlight", id: "cueId-1" }, null, 2)
      )
    );
    track.addCue(
      new VTTCue(
        1,
        2,
        JSON.stringify({ cmd: "highlight", id: "cueId-2" }, null, 2)
      )
    );
    track.addCue(
      new VTTCue(
        2,
        3,
        JSON.stringify({ cmd: "highlight", id: "cueId-3" }, null, 2)
      )
    );
    track.addCue(
      new VTTCue(
        3,
        4,
        JSON.stringify({ cmd: "highlight", id: "cueId-4" }, null, 2)
      )
    );
