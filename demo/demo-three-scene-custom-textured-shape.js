...
class ThreeSceneTexturedShapeDemo extends DemoBase {
  constructor(parentEl) {
    super(
      parentEl,
      [],
      "ThreeScene | Textured shape UV animation",
      "three-scene-textured-shape",
      "Custom textured shape w/UV & vertex displacement on the CPU",
      true
    );
  }
  ...
  updateUVs() {
    ...
    for (let i = 0; i < uvArrayOrig.length; i += 2) {
      ...
      uvArrayCur[i] =
        u +
        this.textureOffsetAmp *
          (0.5 + 0.5 * Math.sin(x / 100 + this.frameCount / 100));
      uvArrayCur[i + 1] =
        v +
        this.textureOffsetAmp *
          (0.5 + 0.5 * Math.sin(y / 100 + this.frameCount / 120));
      vertIndex++;
    }
    ...
    for (let i = 0; i < positionArrayCur.length; i += 3) {
      ...
      positionArrayCur[i + 1] =
        y +
        6 * Math.sin((z / 200) * MathUtil.TWO_PI * 2 + this.frameCount / 10);
      positionArrayCur[i + 2] =
        z + 10 * Math.sin(x / 400 + this.frameCount / 10);
    }
...
if (window.autoInitDemo)
  window.demo = new ThreeSceneTexturedShapeDemo(document.body);
