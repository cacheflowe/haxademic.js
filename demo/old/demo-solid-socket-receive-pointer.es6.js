class SolidSocketDemo extends DemoBase {
  ...
  addClicks() {
    // create buttons
    for (let i = 0; i < 30; i++) {
      document.getElementById("buttons").innerHTML += `<button id="button${
        i + 1
      }">Button ${i + 1}</button>`;
    }
  ...
  remoteClick() {
    PointerUtil.clickDocumentAtPoint(
      this.pointerX.value(),
      this.pointerY.value()
    );
  }
  ...
  frameLoop(frameCount) {
    ...
    this.pointerEl.style.setProperty(
      "transform",
      `translate3d(${this.pointerX.value()}px, ${this.pointerY.value()}px, 0) scale(${this.pointerScale.value()})`
    );
  }
  ...
  initSocket() {
    this.solidSocket = new SolidSocket(
      "ws://192.168.1.3:3001?roomId=987654321&clientType=kiosk"
    );
  ...
  onMessage(msg) {
    ...
    if (json.keyCode) {
      ...
      if (this.inputEl) {
        KeyboardUtil.keyPressSimulateOnTextfield(
          this.inputEl,
          json.keyCode,
          json.character
        );
