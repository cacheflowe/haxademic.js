...
class URLUtilDemo extends DemoBase {
  ...
  init() {
    ...
    this.el.innerHTML = `
      ...
      <div><code>getQueryParam('param')</code> = ${URLUtil.getQueryParam(
        "param"
      )}</div>
      <div><code>getHashQueryParam('test')</code> = ${URLUtil.getHashQueryParam(
        "test"
      )}</div>
      <div><code>getHashQueryVariable('test')</code> = ${URLUtil.getHashQueryVariable(
        "test"
      )}</div>
      <div><code>window.location.href</code> = ${window.location.href}</div>
      <div><code>window.location.protocol</code> = ${
        window.location.protocol
      }</div>
      <div><code>window.location.hostname</code> = ${
      ...
      <div><code>window.location.ancestorOrigins</code> = ${
