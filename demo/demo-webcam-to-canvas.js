...
import CanvasFilters from "../src/canvas-filters.js";
...
class WebcamToCanvasDemo extends DemoBase {
  constructor(parentEl) {
    super(
      ...
      [],
      "Webcam To Canvas",
      "webcam-to-canvas-container",
      "Cropping and color correction of webcam stream"
    );
  }
  ...
  initWebcam() {
    ...
    this.webcam = new Webcam(
      (videoEl) => {
        ...
        this.el.appendChild(
          DOMUtil.stringToDomElement("<h4>Cropped canvas</h4>")
        );
        this.el.appendChild(this.canvasEl);
        this.el.appendChild(
          DOMUtil.stringToDomElement("<h4>Video source</h4>")
        );
  ...
  animate() {
    ...
    if (!this.cropData && this.videoEl.videoWidth > 0) {
      this.cropData = ImageUtil.getOffsetAndSizeToCrop(
        this.canvasEl.width,
